...
namespace ActualCannibalShiaLabeouf
{
	...
	public class IncidentWorker_ShiaSurprise : IncidentWorker
	{
		protected override bool TryExecuteWorker(IncidentParms parms)
		{
			...
			{             
				var str = (current2.equipment == null || current2.equipment.Primary == null) ? "unarmed" : current2.equipment.Primary.LabelCap;
				stringBuilder.AppendLine(current2.KindLabel + " - " + str);
			}

            PawnRelationUtility.Notify_PawnsSeenByPlayer(list, out var pawnRelationsInfo, false, true);
            Find.LetterStack.ReceiveLetter(Consts_ShiaSurprise.letterLabel, Consts_ShiaSurprise.letterText, DefOfs_ShiaSurprise.ThreatSmall, target, parms.faction,null, null, stringBuilder.ToString());
		...
		protected bool TryResolveRaidFaction(IncidentParms parms)
		{
			...
			if (parms.faction == null)
			{
				...
				Faction newfac = FactionGenerator.NewGeneratedFaction(DefOfs_ShiaSurprise.HollywoodCannibals);
	...
	*/
			...
			typeof(Pawn_StoryTracker).GetField("headGraphicPath", BindingFlags.Instance | BindingFlags.NonPublic).SetValue(pawn.story,Consts_ShiaSurprise.path);
            ...
            BackstoryDatabase.TryGetWithIdentifier(childB,out Backstory bsc);
            pawn.story.childhood = bsc;
            BackstoryDatabase.TryGetWithIdentifier(adultB,out Backstory bsa);
